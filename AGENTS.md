...
Default done-when for agent work:

```bash
npm test
```

Run e2e manually when you want browser coverage:

```bash
npm run test:e2e
```

For both together, invoke the `habit-tracker-ship` skill explicitly.
...
- **Unit tests** — `npm test`. API tests only; no external server. This is the default done-when check.
- **E2e tests** — `npm run test:e2e` only when the user asks or you invoke `habit-tracker-ship`. Each suite calls `startServer({ port: 0 })` and uses a temp SQLite file in `os.tmpdir()`. Do not run `npm start` separately.
