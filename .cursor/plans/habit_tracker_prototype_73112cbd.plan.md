...
todos:
  - id: scaffold
    content: Create package.json with express, drizzle-orm, and better-sqlite3; start/test scripts (Node --experimental-strip-types); plus .gitignore
    status: pending
  - id: backend
    content: "Write server.ts: Drizzle/SQLite setup (habits, completions tables), 4 REST routes, static serving of public/, and server-side streak + last-7-days computation"
    status: pending
  - id: frontend
...
## Stack
- Backend: Node v26+ + TypeScript (via `--experimental-strip-types`, no `tsc`/tsx dep) + [Express](https://expressjs.com/) + [Drizzle ORM](https://orm.drizzle.team/) + [`better-sqlite3`](https://github.com/WiseLibs/better-sqlite3) (synchronous, file-based SQLite driver)
- Frontend: one `public/index.html` with inline CSS and vanilla JS using `fetch`
- DB file: `habits.db` (gitignored)

## Data model
Two tables in SQLite (defined in `schema.ts`):
...
## Files to create
- `package.json` - deps (`express`, `drizzle-orm`, `better-sqlite3`), `start`/`test` scripts using `node --experimental-strip-types`
- `tsconfig.json` - TypeScript config for editor/type-checking (no compile step; Node runs `.ts` directly)
- `schema.ts` - Drizzle table definitions (`habits`, `completions`)
- `db.ts` - opens DB, creates tables if missing, Drizzle query helpers
- `server.ts` - Express app, the 4 routes, serves `public/`
